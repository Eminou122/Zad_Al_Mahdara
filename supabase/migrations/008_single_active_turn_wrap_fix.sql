-- Gate 6.4: Single Active Member Wrap Fix
-- Apply manually: Supabase Dashboard -> SQL Editor -> Run
-- Additive-only replacement functions. Does NOT modify 001-007.
...
begin
  ...
  -- Picker rule: current_position if eligible, otherwise first eligible after anchor,
  -- otherwise wrap to first eligible. With one eligible member, wrap returns them.
...
begin
  ...
  -- Wrap includes the completed member when they are the only eligible member.
  if v_next_mid is null then
    select tm.id, tm.position into v_next_mid, v_next_pos
    from team_members tm
    where tm.team_id = v_turn.team_id
