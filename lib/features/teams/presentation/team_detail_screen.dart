...
class _TeamDetailScreenState extends State<TeamDetailScreen> with RouteAware {
  ...
  Future<void> _removeTeam() async {
    ...
    final removed = await zadPermanentDeleteConfirm(
      context,
      count: 1,
      title: 'حذف الفريق نهائياً',
      body:
          'سيتم حذف الفريق وبياناته نهائياً، ولا يمكن استعادتها بعد ذلك. هل تريد المتابعة؟',
      onConfirm: () async {
        setState(() => _teamLifecycleBusy = true);
        try {
          await _svc.deleteTeamPermanently(widget.teamId);
        } finally {
          if (mounted) setState(() => _teamLifecycleBusy = false);
        }
      },
      errorText: (_) => 'حدث خطأ — تحقق من اتصالك بالإنترنت',
    );
                  ...
                  ZadAnimatedEntry(
                    ...
                    child: _TurnCard(
                      ...
                      isAssignedMember:
                          ...
                          d.members.any(
                            (m) =>
                                m.memberId == _turnState!.todayTurn!.memberId &&
                                m.profileId != null &&
